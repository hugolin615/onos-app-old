...
public class ReactiveForwarding {
    ...
    private class ReactivePacketProcessor implements PacketProcessor {
        ...
        public void process(PacketContext context) {
            ...
            if ( (BEGIN == true) && (ethPkt.getEtherType() == Ethernet.TYPE_IPV4)) {
                ...
                //log.info("inpacket {} {} {} {}", dpidStr, srcIp, dstIp, inport);
                //if (dpidStr.equals("0f:00000000000b")) {
                if (dpidStr.equals("of:0000000000000003")) {
                    /*
                    if (ip2port2.containsKey(srcIp)) {
                        //PortNumber outport = ip2port.get(srcIp);
                    ...
                    */
                    ...
                    if (ipv4Protocol == IPv4.PROTOCOL_TCP) {
                        ...
                        if ((tcpDstPort == DEFAULT_DNP3_PORT) || (tcpDstPort == (DEFAULT_DNP3_PORT + 1))) {
                            ...
                            //// handle DNP3 request
                            if ((dnp3B.length > 0) && (!accip.contains(dstIp))) {
                        ...
                        if ((tcpSrcPort == DEFAULT_DNP3_PORT) || (tcpSrcPort == (DEFAULT_DNP3_PORT + 1))) {

                            IPacket dnp3P = tcpPacket.getPayload();
                            byte[] dnp3B = dnp3P.serialize();
                            //log.info("Hui Lin found dnp3 response {}", dnp3B);
                            //log.info("Hui Lin found dnp3 response {}");
                            if ((dnp3B.length > 0) && (!accip.contains(srcIp))) {

                                /// only handle read response at this moment
                                //log.info("Hui Lin responses to inacc {} {}", dnp3B[12], dnp3B.length);
                                //log.info("Hui Lin decoy {} -> {}", srcIp, dstIp);
                                int dnp3fc = (dnp3B[12]) & 0x00FF;
                                log.info("Hui Lin responses to inacc {} {}", dnp3fc, dnp3B.length);
                                if ((dnp3fc == 0x81) && (dnp3B.length > 17)) {
                                    /// dnp3B.length > 17 elminiate response to write request
                                    ...
                                    if (true) {
                                        ...
                                        */
                                        decoy.add(1.0*(srcIp-IPv4.toIPv4Address("10.0.0.1") + 1));
                                        //decoy.add(10.0);
                                        decoy.add(decoydata.get(srcIp - IPv4.toIPv4Address("10.0.0.1")));
                                        decoy.add(10.0);
                                        log.info("Hui Lin call DNP3Out");
                                        //outport = PortNumber.FLOOD;
                                        //packetOut(context, outport);
                                        DNP3Out(context, dnp3B, PortNumber.portNumber(1), decoy);
                                        return;
                ...
                if (dpidStr.equals("of:0000000000000001")) {
                    /*
                    if (ip2port2.containsKey(srcIp)) {
                        //PortNumber outport = ip2port.get(srcIp);
                        packetOut(context, PortNumber.portNumber(1));
                                ...
                                if ((dnp3fc == 0x81) && (dnp3B.length > 17)) {
                                    ...
                                    if (true) {
                                        ...
                                        */
                                        decoy.add(1.0*(srcIp-IPv4.toIPv4Address("10.0.0.1") + 1));
                                        //decoy.add(10.0);
                                        decoy.add(realdata.get(srcIp - IPv4.toIPv4Address("10.0.0.1")));
                                        decoy.add(10.0);
                                        log.info("Hui Lin call DNP3Out");
                                        //outport = PortNumber.FLOOD;
                                        //packetOut(context, outport);
                                        DNP3Out(context, dnp3B, PortNumber.portNumber(2), decoy);
                                        return;
