...
public class AppComponent {
    ...
    public void activate(ComponentContext context) {
        ...
        appId = coreService.registerApplication("org.hlin33.fwd2");
    ...
    private void requestIntercepts() {
        log.info("Hui Lin requestIntercepts");
    ...
    private void readComponentConfiguration(ComponentContext context) {
        log.info("readComponentConfiguration");
    ...
    private class ReactivePacketProcessor implements PacketProcessor {
        ...
        public void process(PacketContext context) {
            ...
            */
            ...
            //System.out.println("handling inbound packet");
            ...
            */
            ...
            log.info("************************");
            log.info("Hui Lin: Handling inbound packet {} from {} to {}", dpid, IPv4.toIPv4AddressBytes(srcIp), IPv4.toIPv4AddressBytes(dstIp));
            //log.info("Hui Lin: Handling inbound packet");
            //log.info("Hui Lin: mac2port: {}", mac2port.keySet());

            //mac2port.putIfAbsent(dpid.toString(),Maps.newHashMap());
            ip2port.putIfAbsent(dpidStr, Maps.newHashMap());

            Map<Integer, PortNumber> temp = ip2port.get(dpidStr);

            if (!temp.containsKey(srcIp)){
                if (temp.containsKey(dstIp)){
                    if(!temp.get(dstIp).equals(inport)){
                        temp.put(srcIp, inport);
                    }
                }
                else{
                    temp.put(srcIp, inport);
            ...
            /*
            Map<String, PortNumber> temp = mac2port.get(dpid.toString());
            ...
            */
            //temp.putIfAbsent(srcId.toString(), inport);

            if (temp.containsKey(dstIp)){
                outport = temp.get(dstIp);
                ...
                for (Integer key : temp.keySet()) {
                   //System.out.println(key + " " + temp.get(key).toString());
                    log.info("{} -> {} ", IPv4.toIPv4AddressBytes(key), temp.get(key));

                }
            }

            log.info("Hui Lin send to port : {}", outport);
            ...
            */
            ...
            if (dst == null) {
                System.out.println("Flood to unknown hosts");
                flood(context);
                return;
            }
            ...
            if (paths.isEmpty()) {
                ...
                System.out.println("Flood as we don't have any path");
                flood(context);
                return;
            }
            ...
            if (path == null) {
                ...
                System.out.println("Flood as we don't know where to go");
                flood(context);
                return;
            }
